...
#exp(dist) used as Weight
    def detect_ir1(self):
        ...
        self.ir_array[1][0] = (self.ir_1_dist.range*(math.cos(self.ir_1_angle)) + self.ir_1_pose[0] ) #Weighted according to the distance
        self.ir_array[1][1] = (self.ir_1_dist.range*(math.sin(self.ir_1_angle)) + self.ir_1_pose[1]) #Weighted according to the distance
        #self.abs_range[1][0] = self.abs_range[1][0] + self.ir_1_dist.range

    def detect_ir2(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[2][0] = (self.ir_2_dist.range*(math.cos(self.ir_2_angle)) + self.ir_2_pose[0] ) #Weighted according to the distance
        self.ir_array[2][1] = (self.ir_2_dist.range*(math.sin(self.ir_2_angle)) + self.ir_2_pose[1]) #Weighted according to the distance
        #self.abs_range[2][0] = self.abs_range[2][0] + self.ir_2_dist.range

    def detect_ir3(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[3][0] = (self.ir_3_dist.range*(math.cos(self.ir_3_angle)) + self.ir_3_pose[0] ) #Weighted according to the distance
        self.ir_array[3][1] = ( self.ir_3_dist.range*(math.sin(self.ir_3_angle)) + self.ir_3_pose[1]) #Weighted according to the distance
        #self.abs_range[3][0] = self.abs_range[3][0] + self.ir_3_dist.range

    def detect_ir4(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[4][0] = (self.ir_4_dist.range*(math.cos(self.ir_4_angle)) + self.ir_4_pose[0] ) #Weighted according to the distance
        self.ir_array[4][1] = (self.ir_4_dist.range*(math.sin(self.ir_4_angle)) + self.ir_4_pose[1]) #Weighted according to the distance
        #self.abs_range[4][0] = self.abs_range[4][0] + self.ir_4_dist.range

    def detect_ir5(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[5][0] = (self.ir_5_dist.range*(math.cos(self.ir_5_angle)) + self.ir_5_pose[0] ) #Weighted according to the distance
        self.ir_array[5][1] = (self.ir_5_dist.range*(math.sin(self.ir_5_angle)) + self.ir_5_pose[1]) #Weighted according to the distance
        #self.abs_range[5][0] = self.abs_range[5][0] + self.ir_5_dist.range

    def detect_ir6(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[6][0] = ( self.ir_6_dist.range*(math.cos(self.ir_6_angle)) + self.ir_6_pose[0] ) #Weighted according to the distance
        self.ir_array[6][1] = (self.ir_6_dist.range*(math.sin(self.ir_6_angle)) + self.ir_6_pose[1]) #Weighted according to the distance
        #self.abs_range[6][0] = self.abs_range[6][0] + self.ir_6_dist.range

    def detect_ir7(self):
        abs_range = 0
        # if self.ir_1_dist.range <=0.15(self):#Considering only of distance is less than 0.15m
        self.ir_array[7][0] = (self.ir_7_dist.range*(math.cos(self.ir_7_angle)) + self.ir_7_pose[0] ) #Weighted according to the distance
        self.ir_array[7][1] = (self.ir_7_dist.range*(math.sin(self.ir_7_angle)) + self.ir_7_pose[1]) #Weighted according to the distance
    ...
    def detect_ir8(self):
        ...
        self.ir_array[8][0] = (self.ir_8_dist.range*(math.cos(self.ir_8_angle)) + self.ir_8_pose[0] ) #Weighted according to the distance
        self.ir_array[8][1] = (self.ir_8_dist.range*(math.sin(self.ir_8_angle)) + self.ir_8_pose[1]) #Weighted according to the distance
    ...
    def change_coordinate(self):
        ...
        q = [ self.im.pose.pose.orientation.x,
                self.im.pose.pose.orientation.y,
                self.im.pose.pose.orientation.z,
                self.im.pose.pose.orientation.w ]
        eu = tf.transformations.euler_from_quaternion(q)
        theta = (round(eu[2]*180/3.1415,2))
        ...
        for i in range(1,8):
       # Change cordinate for ith sensor
    ...
    def ch_coo(self):
        for i in [1,2,3,4,5,6,7,8]:
            self.ir_array[i][0] = self.ir_array[i][0] + self.bot_pose.point.x
            self.ir_array[i][1] = self.ir_array[i][1] + self.bot_pose.point.y
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        rospy.Subscriber('/odom', Odometry, obs.imu_read)
        obs.read_all_ir()
